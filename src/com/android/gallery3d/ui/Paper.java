...
class Paper {
    ...
    public float[] getTransform(Rect rect, float scrollX,float zoffset) {
        float left = mAnimationLeft.getValue();
        float right = mAnimationRight.getValue();
        float screenX = rect.centerX() - scrollX;
        // We linearly interpolate the value [left, right] for the screenX
        ...
        float degrees =
                (1 / (1 + (float) Math.exp(-t * ROTATE_FACTOR)) - 0.5f) * 2 * -45;
        Matrix.setIdentityM(mMatrix, 0);
        Matrix.translateM(mMatrix, 0, mMatrix, 0, rect.centerX(), rect.centerY(), zoffset);
        Matrix.rotateM(mMatrix, 0, degrees, 0, 1, 0);
        Matrix.translateM(mMatrix, 0, mMatrix, 0, -rect.width() / 2, -rect.height() / 2, -zoffset);
        return mMatrix;
    }
    //end
}
