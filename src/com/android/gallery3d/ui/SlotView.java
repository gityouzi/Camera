...
public class SlotView extends GLView {
    ...
    public interface Listener {
        ...
        //jiangxd add for gesture delete item
        public void onGestureLongTap(int index);
        //end
    }
    ...
    public static class SimpleListener implements Listener {
		...
		@Override public void onGestureLongTap(int index) {}
    }
    ...
    private int renderItem(
      ...
      //jiangxd add for ycservice
        float zoffset = 0;
        if(mHasHands){
        	float mScrollx = mLayout.mScrollPosition+mLayout.mWidth/2;
        	int mItemCenter = (rect.left+rect.right)/2;
        	float effectArea = (mLayout.mSlotWidth + mLayout.mSlotGap);
	    	if(Math.abs(mScrollx-mItemCenter) <= effectArea){
	    		float changeValue = 1.0f-Math.abs(mScrollx-mItemCenter)/effectArea;
	    		canvas.setAlpha(0.3f+0.7f*changeValue);
	    		zoffset = -300f*changeValue;
	    	}else{
	    		zoffset = 0;
    	...
    	//end
        if (paperActive) {
            canvas.multiplyMatrix(mPaper.getTransform(rect, mScrollX ,zoffset), 0);
        } else {
            canvas.translate(rect.left, rect.top, zoffset);
        }
...
//            padding[1] = Math.max(0, (majorLength - mContentLength) / 2);
        ...
        private void updateVisibleSlotRange() {
            ...
            if (WIDE) {
            	//jiangxd modify for ycservice
            	mCurrentSlot = position  / (mSlotWidth + mSlotGap);
            	int startCol = (position - mWidth/2) / (mSlotWidth + mSlotGap);
...
//        	return limit <= 0 ? 0 : limit;
            int limit = WIDE ? mContentLength : mContentLength - mSlotHeight;
            return limit <= -mWidth/2 ? -mWidth/2 : limit;
    ...
    private class MyGestureListener implements GestureDetector.OnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1,
            ...
            cancelDown(false);
            //jiangxd add for ycservice
//            if(distanceX<10 && distanceX>-10 && distanceY>20){
//            	int index = mLayout.mCurrentSlot;
...
//            }
    ...
    //jiangxd add for ycservice
    public void scrollbyYCpoint(int distance){
    	int overDistance = mScroller.startScroll(
                Math.round(distance), 0, mLayout.getScrollLimit());
        invalidate();
    }
    public void openslotByYC(){
    	int index = mLayout.mCurrentSlot;
    	if (index != INDEX_NONE) mListener.onSingleTapUp(index);
    }
    //end
	...
	private boolean mHasHands = false;
	public void detectedHands(boolean mhand){
		mHasHands = mhand;
	}
	public void onGestureFling(int event){
		if(!mHasHands){
			mHasHands = true;
		}
        switch(event){
        case IYCServiceCallback.SINGLE_TAP:
        	if (mLayout.mCurrentSlot != INDEX_NONE){
        		mListener.onSingleTapUp(mLayout.mCurrentSlot);
        	}
        	break;
        case IYCServiceCallback.LONG_PRESS:
        	if (mLayout.mCurrentSlot != INDEX_NONE){
        		mListener.onGestureLongTap(mLayout.mCurrentSlot);
        	}
        	break;
        }
...
//        if (mUIListener != null) mUIListener.onUserInteractionBegin();
	...
	//end
	private void onPositionScroll(boolean isLeft){
		if(isLeft){
			mScroller.startScroll(
	                -50, 0, mLayout.getScrollLimit());
			invalidate();
		}else{
			mScroller.startScroll(
	                50, 0, mLayout.getScrollLimit());
	...
	public void onPositionChangedExt(int offsetX, int isEdged) {
		if(isEdged ==0){
		mScroller.startScroll(
	                Math.round(offsetX), 0, mLayout.getScrollLimit());
		invalidate();
		}
		if(isEdged ==1){
			mScroller.startScroll(
	                -20, 0, mLayout.getScrollLimit());
			invalidate();
		}
		if(isEdged ==2){
			mScroller.startScroll(
    ...
    //jiangxd add for onTouchgesture
