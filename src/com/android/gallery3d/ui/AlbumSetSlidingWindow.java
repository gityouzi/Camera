...
public class AlbumSetSlidingWindow implements AlbumSetDataLoader.DataListener {
    ...
    public static class AlbumSetEntry {
        ...
        public MediaItem[] coverAddItem;//jiangxd add
        public Texture content;
        public Texture[] contentAdd;//jiangxd add
        public BitmapTexture labelTexture;
        public TiledTexture bitmapTexture;
        public TiledTexture[] bitmapAddTexture;//jiangxd add
        public Path setPath;
        public String title;
        ...
        public boolean isWaitLoadingDisplayed2;//jiangxd add
        public boolean isWaitLoadingDisplayed3;//jiangxd add
        public boolean isWaitLoadingDisplayed4;//jiangxd add
        public long setDataVersion;
        public long coverDataVersion;
        private BitmapLoader labelLoader;
        private BitmapLoader coverLoader;
        private BitmapLoader coverLoader2;//jiangxd add
        private BitmapLoader coverLoader3;//jiangxd add
        private BitmapLoader coverLoader4;//jiangxd add
    }
...
//        if (entry.coverAddLoader!=null) entry.coverAddLoader.startLoad();//jiangxd add
        //jiangxd add
        if (entry.coverLoader2 != null) entry.coverLoader2.startLoad();
        if (entry.coverLoader3 != null) entry.coverLoader3.startLoad();
        if (entry.coverLoader4 != null) entry.coverLoader4.startLoad();
        //end
...
//        if (entry.coverAddLoader!=null) entry.coverAddLoader.cancelLoad();//jiangxd add
        //jiangxd add
        if (entry.coverLoader2 != null) entry.coverLoader2.cancelLoad();
        if (entry.coverLoader3 != null) entry.coverLoader3.cancelLoad();
        if (entry.coverLoader4 != null) entry.coverLoader4.cancelLoad();
        //end
...
//        if (entry.coverAddLoader != null) entry.coverAddLoader.recycle();//jiangxd add
        //jiangxd add
        if (entry.coverLoader2 != null) entry.coverLoader2.recycle();
        if (entry.coverLoader3 != null) entry.coverLoader3.recycle();
        if (entry.coverLoader4 != null) entry.coverLoader4.recycle();
        for(int i=0;i<3;i++){
        	if(entry.bitmapAddTexture[i]!=null) entry.bitmapAddTexture[i].recycle();
        }
        //end
    ...
    private void updateAlbumSetEntry(AlbumSetEntry entry, int slotIndex) {
        ...
        MediaItem[] coverAdd = mSource.getCoverAddItem(slotIndex);//jiangxd add
        ...
        //jiangxd add
        entry.coverAddItem = coverAdd;
        entry.contentAdd = new Texture[3];
        entry.bitmapAddTexture = new TiledTexture[3];
        //end
        if (getDataVersion(cover) != entry.coverDataVersion) {
            ...
            //jiangxd add
            if(entry.coverLoader2!=null){
            	entry.coverLoader2.recycle();
            	entry.coverLoader2 =null;
            }
            if(entry.coverLoader3!=null){
            	entry.coverLoader3.recycle();
            	entry.coverLoader3 =null;
            }
            if(entry.coverLoader4!=null){
            	entry.coverLoader4.recycle();
            	entry.coverLoader4 =null;
            }
            //end
            if (cover != null) {
                entry.coverLoader = new AlbumCoverLoader(slotIndex, cover,0);
            }
            for(int i=0;i<coverAdd.length;i++){
            	if(coverAdd[i] != null && i==0){
            		entry.coverLoader2 = new AlbumCoverLoader(slotIndex, coverAdd[0],1);
            	}
            	if(coverAdd[i] != null && i==1){
            		entry.coverLoader3 = new AlbumCoverLoader(slotIndex, coverAdd[1],2);
            	}
            	if(coverAdd[i] != null && i==2){
            		entry.coverLoader4 = new AlbumCoverLoader(slotIndex, coverAdd[2],3);
            	}
            }
        }
    }
    ...
    private void uploadBackgroundTextureInSlot(int index) {
        ...
        //jiangxd add
        for(int i=0;i<3;i++){
        	if(entry.bitmapAddTexture[i]!=null){
        		mContentUploader.addTexture(entry.bitmapAddTexture[i]);
        	}
        }
        //end
    ...
    private void updateTextureUploadQueue() {
        ...
        for (int i = mActiveStart, n = mActiveEnd; i < n; ++i) {
            ...
            //jiangxd add
            for(int j=0;j<3;j++){
            	if (entry.bitmapAddTexture[j] != null) {
                    mContentUploader.addTexture(entry.bitmapAddTexture[j]);
                }
            }
            if (entry.labelTexture != null) {
...
//            if (startLoadBitmapAdd(entry.coverAddLoader)) ++mActiveRequestCount;
            //jiangxd add 
            if (startLoadBitmap(entry.coverLoader2)) ++mActiveRequestCount;
            if (startLoadBitmap(entry.coverLoader3)) ++mActiveRequestCount;
            if (startLoadBitmap(entry.coverLoader4)) ++mActiveRequestCount;
            //end
        }
    ...
    private class AlbumCoverLoader extends BitmapLoader implements EntryUpdater {
        ...
        private int mIndex = 0;//jiangxd add
        public AlbumCoverLoader(int slotIndex, MediaItem item ,int index) {
...
//            mHandler.obtainMessage(MSG_UPDATE_ALBUM_ENTRY, this).sendToTarget();
            Message msg = mHandler.obtainMessage(MSG_UPDATE_ALBUM_ENTRY, this);
            msg.arg1 = mIndex;
            msg.sendToTarget();
        }
        ...
        public void updateEntry() {
            ...
            if(mIndex==0){
            	...
            	entry.bitmapAddTexture[mIndex-1]= texture;
            	entry.contentAdd[mIndex-1]=texture;
            }
//            entry.bitmapTexture = texture;
//            entry.content = texture;
