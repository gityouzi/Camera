...
public class AlbumSetPage extends ActivityState implements
    ...
    private ResourceTexture mUpBgOverlay;
    private ResourceTexture mDownBgOverlay;
    private int ycState = 0;
    private static final int YC_MODE_IDLE = 0;
    private static final int YC_MODE_SELECT = 1;
    //end
    ...
    private final GLView mRootPane = new GLView() {
        ...
        protected void onLayout(
            ...
            //jiangxd add for ycservice
            layoutPaddingTop = slotViewTop;
            layoutPaddingBottom = slotViewBottom;
            //end
            if (mShowDetails) {
    ...
    public void onCreate(Bundle data, Bundle restoreState) {
        ...
        //jiangxd for ycservice
        mUpBgOverlay = new ResourceTexture(mActivity,
                R.drawable.ic_invision_up_overlay);
        mDownBgOverlay = new ResourceTexture(mActivity,
                R.drawable.ic_invision_up_overlay);
    }
...
//        mActionModeHandler.updateSupportedOperation(path, selected);
    ...
    //jiangxd add for ycservice
    public void setYCstate(int mState){
    	ycState = mState;
    }
    public int getYCstate(){
    	return ycState;
    }
    //end
    //jiangxd add for ycservice
    private static int currentMode = 0;
    private static final int ENTRY_SCROLL_MODE = 1;
    private int scrollX = -1;
    private int scrollY = -1;
    private static int layoutPaddingTop;
    private static int layoutPaddingBottom;
    
	@Override
	public void onHandsDetected(boolean mHasHand) {
//		if(!mHasHand){
//			currentMode = 0;
//		}
		mSlotView.detectedHands(mHasHand);
	}

	@Override
	public void onPositionChanged(int x, int y) {
//		if(currentMode == ENTRY_SCROLL_MODE){
//			if(y<layoutPaddingBottom && y>layoutPaddingTop ){
//				int distance = scrollX-x;
//				mSlotView.scrollbyYCpoint(distance);
//				scrollX = x;
//				scrollY =y;
//			}
//			if(y<layoutPaddingTop){
//				mSlotView.openslotByYC();
//            	currentMode = 0;
//                return ;
//			}
//			if(y>layoutPaddingBottom){
//				currentMode = 0;
//                return ;
//			}
//		}
...
//		}
	...
	//end
	@Override
	public void onGestureEvent(int mGesture) {
		mSlotView.onGestureFling(mGesture);
	}
	public void onPositionChangedExt(int offsetX,int isEdged){
		mSlotView.onPositionChangedExt(offsetX,isEdged);
	}
	public void onTouchScroll(int arg0,float x,float y){
		mSlotView.detectedHands(true);
		if(Math.abs(x)>Math.abs(y)){
			mSlotView.onPositionChangedExt((int)x,0);
		}else{
			mSlotView.onPositionChangedExt((int)y,0);
		}
	}
	public void onTouchGesture(int action, float x, float y){
		if(action == MotionEvent.ACTION_UP){
			mSlotView.onGestureFling(5);
		}
	};
	public void onTouchFling(float velocityX, float velocityY){
        mSlotView.onTouchFling(velocityX,velocityY);
	};
}
