...
public class StateManager {
    ...
    public void onGestureEvent(int mGesture){
    	if (!mStack.isEmpty()) getTopState().onGestureEvent(mGesture);
    }
    public void onHandsDetected(boolean mHands){
    	if (!mStack.isEmpty()) getTopState().onHandsDetected(mHands);
    }
    public void onPositionChanged(int x, int y) {
    	if (!mStack.isEmpty()) getTopState().onPositionChanged(x,y);
    }
    public void onPositionChangedExt(int offsetx, int isEdged) {
    	if (!mStack.isEmpty()) getTopState().onPositionChangedExt(offsetx,isEdged);
    }
    public void onTouchScroll(int arg0,float x,float y){
    	if (!mStack.isEmpty()) getTopState().onTouchScroll(arg0,x,y);
    }
    public void onTouchGesture(int action, float x, float y){
    	if (!mStack.isEmpty()) getTopState().onTouchGesture(action,x,y);
	};
	public void onTouchFling(float velocityX, float velocityY){
		if (!mStack.isEmpty()) getTopState().onTouchFling(velocityX,velocityY);
	};
    //end
    public void resume() {
    ...
    public void	onPreBtnClick(){
    	if (!mStack.isEmpty()) getTopState().onPreBtnClick();
    };
	public void onNextBtnClick(){
		if (!mStack.isEmpty()) getTopState().onNextBtnClick();
	};
	public void onDeleteBtnClick(){
		if (!mStack.isEmpty()) getTopState().onDeleteBtnClick();
	};
    //end
}
