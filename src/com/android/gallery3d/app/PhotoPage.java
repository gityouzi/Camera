...
public class PhotoPage extends ActivityState implements
    ...
    public void onPause() {
        ...
        if(isDockBarShowed && mActivity instanceof Gallery){
			((Gallery) mActivity).setDockBarVisible(false);
			isDockBarShowed = false;
		}
        //end
        mIsActive = false;
  ...
  //jiangxd add for ycservice
    private boolean isDockBarShowed = false;
	@Override
	public void onHandsDetected(boolean mHasHand) {
			if(mActivity instanceof Gallery){
				((Gallery) mActivity).setDockBarVisible(mHasHand);
				isDockBarShowed = mHasHand;
			}
			((Gallery) mActivity).dockBarHandsDetected(mHasHand);
	}

	@Override
	public void onPositionChanged(int x, int y) {
		if(!isDockBarShowed && mActivity instanceof Gallery){
			((Gallery) mActivity).setDockBarVisible(true);
			((Gallery) mActivity).dockBarHandsDetected(true);
			isDockBarShowed = true;
		}
	}
	@Override
	public void onGestureEvent(int mGesture) {
		if(!isDockBarShowed){
		if(mGesture==SINGLE_TAP){
			onSingleTapUp(mPhotoView.getWidth()/2,mPhotoView.getHeight()/2);
		}
		}else{
		if(mActivity instanceof Gallery){
			switch(mGesture){
				case MOVE_PREV:
					((Gallery) mActivity).dockBarChangeItem(false);
					break;
				case MOVE_NEXT:
					((Gallery) mActivity).dockBarChangeItem(true);
					break;
					
				case SINGLE_TAP:
					((Gallery) mActivity).dockBarClickItem();
					break;
				default:
					break;
			}
...
//		}
	...
	public void onTouchFling(float x, float y){
...
//	public void onTouchScroll(float x,float y){
//		if(Math.abs(x)>Math.abs(y)){
//			if(x>0){
//				onNextBtnClick();
//			}else{
